function [Q,ets] = windy_gw(alpha,sideII,sideJJ,s_start,s_end,wind,MAX_N_EPISODES)
...
nStates = % TODO: find the number of states
% (hint, how many squares are there on a grid that has side lengths sideII,
% and sideJJ?)
...
Q = % TODO: create a state-action matrix, with values equal to zero
% (hint, the dimensions are (number of states, number of actions)
...
for ei= % TODO: finish this for loop so that it loops a total of MAX_N_EPISODES times
  tic; 
  if( ei==1 ) 
  ...
  while( 1 ) 
    ...
    if( ~( (stp1(1)==s_end(1)) && (stp1(2)==s_end(2)) ) ) % stp1 is not the terminal state
      ...
      Q(sti,at) = Q(sti,at) + alpha* % TODO: multiply alpha with the 
      % difference between the reward, and the value of the state-action pair
      % (hint, the reward is rew, and the value of the state-action pair is
      % Q(sti, at)
      break; 
    end
    %update (st,at) pair: 
